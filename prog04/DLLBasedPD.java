...
public class DLLBasedPD implements PhoneDirectory {
	/**
	 * The head (first entry) and tail (last entry) of the linked list that stores
	 * the directory data
	 */
	...
	/**
	 * Method to load the data file. pre: The directory storage has been created and
	 * it is empty. If the file exists, it consists of name-number pairs on adjacent
	 * lines. post: The data from the file is loaded into the directory.
	 * 
	...
	/**
	 * Method to save the directory. pre: The directory has been loaded with data.
	 * post: Contents of directory written back to the file in the form of
	 * name-number pairs on adjacent lines.
	 */
	public void save() {
		try {
	 ...
	 * 
	 * @param location The location to add the new entry, null to add it at the end
	 *                 of the list
	...
	protected boolean found(DLLEntry location, String name) {
		if (location == null ||location!=find(name))
			return false;
		return true;
	}
