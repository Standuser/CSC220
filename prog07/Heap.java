...
public class Heap <E> extends AbstractQueue<E> {
 ...
 // If it is the last element in the list, just remove it.
    ...
    // item at index.
	  ...
	  theData.remove(size()-1);
	  while(!isNull(getParent(index)) && compare(index,getParent(index))<0||!isNull(getLeft(index))&&compare(index, getLeft(index))>0|| !isNull(getRight(index))&& compare(index, getRight(index))>0) {
		  if(compare(index,getParent(index))<0) {
				swap(index,getParent(index));
	    		index=getParent(index);
		  }else
	  if (!isNull(getRight(index))&& compare(getRight(index), getLeft(index))<0) {
