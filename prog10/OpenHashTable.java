...
public class OpenHashTable<K, V> extends AbstractMap<K, V> {
	...
	private int find(Object key) {
		...
		while (table[index] != null && (table[index] == DELETED || !key.equals(table[index].key))) {
			if (firstDelete == -1 && table[index] == DELETED)
				firstDelete = index;
			index = (index + 1) % table.length;

		}
		if (table[index] != null || firstDelete == -1)
			return index;
		return firstDelete;
	...
	private void rehash(int newCapacity) {
		...
		for (int i = 0; i < o.length; i++) {
			...
			if (o[i] != null && o[i] != DELETED) {

				put(o[i].key, o[i].value);
	...
	private class EntryIterator implements Iterator<Map.Entry<K, V>> {
		...
		public boolean hasNext() {
			for ( int a = i; a<table.length;a++) {
				if (table[a]!=null&&table[a]!= DELETED) {
					return true;
				}
			}
			// EXERCISE
